...
static void debug_log(const std::string& msg) {
    std::cerr << "[PlannerV2] " << msg << std::endl;
}
...
AggFunc PlannerV2::parseAggFunc(const std::string& name) {
    ...
    return AggFunc::Sum;  // default
}

CompareOp PlannerV2::parseCompareOp(const std::string& op) {
...
TypedExprPtr PlannerV2::parseExpression(const std::string& exprStr) {
    ...
    */
    ...
    // Check for comparison FIRST (before binary arithmetic)
    // Need to check this before arithmetic because date literals like '1998-09-02' contain '-'
    // Also handle !~~ (NOT LIKE) and ~~ (LIKE) operators from DuckDB/PostgreSQL
    ...
    if (std::regex_match(s, m, cmpRe)) {
        ...
        if (op == "~~" || op == "!~~") {
            // Convert to LIKE or NOTLIKE function call
            std::string funcName = (op == "~~") ? "LIKE" : "NOTLIKE";
            FunctionCall func;
            func.name = funcName;
        ...
        return TypedExpr::compare(parseCompareOp(op), parseExpression(left), parseExpression(right));
    }
    ...
    if (funcMatched) {
        ...
        if (funcUpper != "SUM" && funcUpper != "COUNT" && funcUpper != "AVG" &&
            funcUpper != "MIN" && funcUpper != "MAX" && funcUpper != "FIRST" &&
            funcUpper != "DATE") {
            // Parse arguments
            std::string argsStr = m[2].str();
...
static void traverseNode(const json& node, TraverseContext& ctx) {
    ...
    if (name == "CTE") {
        if (node.contains("children") && node["children"].is_array()) {
            ...
            if (kids.size() >= 2) {
                ...
                ctx.plan.nodes.push_back(IRNodeV2::save(cteName));
    ...
    if (nameLower.find("join") != std::string::npos && 
        ...
        if (kids.size() == 2) {
                    ...
                    if (nl.find("scan") != std::string::npos || nl == "get" || nl.find("read_csv") != std::string::npos || nl.find("delim_scan") != std::string::npos) {
                        ...
                        if (curr.contains("extra_info") && curr["extra_info"].is_object()) {
                            ...
                            if (!tbl.empty()) {
                                ...
                                if (!filterStr.empty()) {
                                    capturedRightFilter = PlannerV2::parseExpression(filterStr);
                                }
            ...
            // Proposed Fix: Handle RHS Capture (Simple & Complex)
            ...
            if (nameLower.find("delim_join") == std::string::npos) {
                if (tryCapture(kids[1])) {
                    ...
                    // Use a temporary context to traverse the RHS and get projections
                    {
                        ...
                        PlanV2 dummyPlan;
                        TraverseContext dummyCtx = fhCtx;
                        dummyCtx.plan = dummyPlan; // Detach from real plan
                    ...
                    childProjs.insert(childProjs.end(), rhsProjections.begin(), rhsProjections.end());   // RHS projs
                } else {
                    ...
                    ctx.plan.nodes.push_back(IRNodeV2::save(saveID));
                    ...
                    childProjs.insert(childProjs.end(), rhsProjections.begin(), rhsProjections.end());   // RHS
                }
            }
        }
    }
        ...
        if (nameLower.find("join") != std::string::npos) {
            ...
            if (kids.size() >= 2) {
                ...
                std::function<bool(const json&)> checkDelim = [&](const json& n) -> bool {
                    ...
                    if (nl.find("delim_scan") != std::string::npos) return true;
                    if (n.contains("children")) {
                        for(const auto& c : n["children"].get_array()) if(checkDelim(c)) return true;
                    }
                    return false;
                };
                hasDelimScanInRHS = checkDelim(kids[1]);

                bool isDelimJoin = (nameLower.find("delim_join") != std::string::npos) || hasDelimScanInRHS;
                if (isDelimJoin) debug_log("Processing DELIM_JOIN at node " + std::to_string(ctx.plan.nodes.size()));

                if (isDelimJoin) {
                    debug_log("Processing DELIM_JOIN. Children: " + std::to_string(kids.size()));
                    // LHS First -> Save -> RHS (with delim) -> Save -> Scan LHS -> Join
                    ...
                    // if (!ctx.plan.nodes.empty() && ctx.plan.nodes.back().type == IRNodeV2::Type::Save) {
                    ...
                    // } else {
                         ...
                         debug_log("Emitting SAVE for DELIM_JOIN LHS: " + lhsSaveID + " at index " + std::to_string(ctx.plan.nodes.size()));
                         ctx.plan.nodes.push_back(IRNodeV2::save(lhsSaveID));
                    ...
                    // Traverse RHS with isolation to capture tables/aliases
                    ...
                    ctx.plan.nodes.push_back(IRNodeV2::save(rhsSaveID));
                    capturedRightTable = rhsSaveID;
                    capturedRHS = true;
                    
                    IRNodeV2 restoreScan = IRNodeV2::scan(lhsSaveID);
                    ...
                    // Hack for Q02: If LHS save missed 'part' table (common with optimization bugs),
                    // Generic DELIM_JOIN Handling: Restore LHS and join with captured RHS
                    ...
                    ctx.plan.nodes.push_back(IRNodeV2::join(JoinType::Inner, PlannerV2::parseExpression(delimCondRaw), delimCondRaw, rhsSaveID, nullptr));
                    ...
                    // Fix: Rename RHS columns to avoid collisions in GPUNativeExecutor context 
                    // and ensure keys are unique.
                    ...
                    if (!renamedRhsProjs.empty()) {
                        rhsCtx.plan.nodes.push_back(IRNodeV2::project(projectExprs, projectNames));
                    ...
                    // Fix: Correct Projection Order (LHS + RHS) for DuckDB #N resolution
                    // DuckDB logic assumes LHS columns come first, then RHS columns.
                    // Previous logic appended RHS then LHS, causing #N to map to potentially wrong columns 
                    // if indices are position-dependent (which they are in resolveColRef).
    ...
    if (nameLower.find("scan") != std::string::npos || 
        ...
        if ((nameLower.find("delim_scan") != std::string::npos || nameLower == "column_data_scan" || isDummy) && !ctx.delimStack.empty()) {
            ...
            if (myProjs.empty()) myProjs = ctx.delimStack.back().second;
             ...
             if (extraInfo.is_object() && extraInfo.contains("Filters")) {
                ...
                if (!filterStr.empty()) {
                    ctx.plan.nodes.push_back(IRNodeV2::filter(PlannerV2::parseExpression(filterStr), filterStr));
                }
             }
             return;
        }

        IRNodeV2 scanNode = IRNodeV2::scan(delimTableOverride);
        ...
        if (extraInfo.is_object() && extraInfo.contains("Filters")) {
            ...
            if (!filterStr.empty()) {
                scan.filter = PlannerV2::parseExpression(filterStr);
            }
        }
    ...
    // ========== FILTER ==========
        ...
        // Fix for Q7: Ambiguous column references in OR predicate (n_name vs n_name_rhs_2 vs n_name_rhs_6)
        // Scan predicate for column names that are NOT in childProjs but have aliased variants.
        // If found, replace them cyclically.
        {
            ...
            for (auto& [word, cands] : candidatesMap) {
                ...
                // Note: we must re-scan 'predicate' which is current state (in case multiple words replaced, though unlikely to overlap)
        ...
        if ((predicate.find("SUBQUERY") != std::string::npos) && !ctx.plan.nodes.empty()) {
             ...
             if (handledByJoin) {
                 ...
                 pos = 0;
                 // Be careful not to match partial words, but SUBQUERY is distinct enough in DuckDB internal gen
                 while ((pos = predicate.find("SUBQUERY", pos)) != std::string::npos) { predicate.replace(pos, 8, "1=1"); }
             }
        }

        IRNodeV2 filterNode = IRNodeV2::filter(PlannerV2::parseExpression(predicate), predicate);
        filterNode.duckdbName = name;
        ctx.plan.nodes.push_back(std::move(filterNode));
    }
    // ========== GROUP_BY ==========
    else if (nameLower.find("group_by") != std::string::npos) {
        IRNodeV2 gbNode = IRNodeV2::groupBy();
        ...
        if (extraInfo.is_object()) {
            ...
            if (extraInfo.contains("Aggregates")) {
                ...
                auto processAgg = [&](std::string agg) {
                    ...
                    spec.func = PlannerV2::parseAggFunc(funcName);
                    ...
                    if (start != std::string::npos && end != std::string::npos) {
                        ...
                        if (isChildColumn) {
                            ...
                            spec.input = PlannerV2::parseExpression(spec.inputExpr);
                        }
                    }
    ...
    // ========== UNGROUPED_AGGREGATE / AGGREGATE ==========
        ...
        if (extraInfo.is_object() && extraInfo.contains("Aggregates")) {
            ...
            std::vector<IRNodeV2> bufferedAggs;
            ...
            for (size_t aggIdx = 0; aggIdx < aggStrings.size(); ++aggIdx) {
                ...
                if (start != std::string::npos && end != std::string::npos) {
                    ...
                    func = PlannerV2::parseAggFunc(funcName);
                    exprStr = trim_str(aggStr.substr(start + 1, end - start - 1));

                    // Fix for potentially nested or misparsed aggregate strings in Q15
                ...
                IRNodeV2 aggNode = IRNodeV2::aggregate(func, PlannerV2::parseExpression(exprStr));
    ...
    // ========== PROJECTION ==========
        ...
        for (const auto& proj : myProjs) {
            ...
            exprs.push_back(PlannerV2::parseExpression(exprStr));
            names.push_back(outName);
        }
        ...
        IRNodeV2 projNode = IRNodeV2::project(std::move(exprs), std::move(names));
    ...
    // ========== ORDER_BY / TOP_N ==========
        ...
        IRNodeV2 obNode = IRNodeV2::orderBy();
    ...
    // ========== LIMIT ==========
        ...
        IRNodeV2 limNode = IRNodeV2::limit(count);
    ...
    // ========== JOIN ==========
        ...
        if (nameLower.find("join") != std::string::npos) {
            std::cerr << "[PlannerV2] Creating JOIN '" << name << "'. CapturedRightTable: '" << capturedRightTable << "'" << std::endl;
            std::cerr << "[PlannerV2] Pre-resolved Condition: '" << condStr << "'" << std::endl;
            std::cerr << "[PlannerV2] ChildProjs size: " << childProjs.size() << std::endl;
        ...
        // Fix for SUBQUERY keyword in Join conditions (scalar subquery comparison)
        if (condStr.find("SUBQUERY") != std::string::npos && !rhsProjections.empty()) {
             // Replace SUBQUERY with the first column of the RHS (assuming scalar subquery result)
             ...
             std::cerr << "[PlannerV2] Replaced SUBQUERY with '" << rhsCol << "' in Join Condition -> " << condStr << std::endl;
        }

        if (nameLower.find("join") != std::string::npos) {
            std::cerr << "[PlannerV2] Resolved Condition: '" << condStr << "'" << std::endl;
        }
        ...
        IRNodeV2 joinNode = IRNodeV2::join(jtype, PlannerV2::parseExpression(condStr), condStr, capturedRightTable, capturedRightFilter);
...
PlanV2 PlannerV2::fromSQL(const std::string& sql) {
    PlanV2 plan;
    ...
    try {
        ...
        for (const auto& n : plan.nodes) {
            if (n.type == IRNodeV2::Type::Limit) { hasLimit = true; break; }
        }
        if (!hasLimit) {
            std::regex re_limit(R"(limit\s+(\d+))", std::regex::icase);
            std::smatch m;
            if (std::regex_search(sql, m, re_limit) && m.size() > 1) {
                plan.nodes.push_back(IRNodeV2::limit(std::stoll(m[1].str())));
            }
        }
...
PlannerV2::Feasibility PlannerV2::checkGPUFeasibility(const PlanV2& plan) {
    Feasibility f;
...
PlannerV2::extractNeededColumns(const PlanV2& plan) {
    ...
    for (const auto& node : plan.nodes) {
        if (node.type == IRNodeV2::Type::Scan) {

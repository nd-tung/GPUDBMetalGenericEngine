            ...
            if (kids.size() >= 2) {
                ...
                if (isDelimJoin) {
                    ...
                    // } else {
                         ...
                         auto saveNode = IRNodeV2::save(lhsSaveID);
                         debug_log("Created Save Node with type: " + std::to_string((int)saveNode.type));
                         ctx.plan.nodes.push_back(std::move(saveNode));
                         if (ctx.plan.nodes.back().type == IRNodeV2::Type::Save) debug_log("CONFIRMED: Back node is Save.");
                    ...
                    debug_log("DELIM_JOIN emitting join type: " + std::to_string(static_cast<int>(delimJoinType)) + 
                              " (0=Inner, 4=Semi, 5=Anti)");
                    
                    ctx.plan.nodes.push_back(IRNodeV2::join(delimJoinType, PlannerV2::parseExpression(delimCondRaw), delimCondRaw, rhsSaveID, nullptr));
                    ...
                    // Note: rhsProjections already renamed above with unique suffix

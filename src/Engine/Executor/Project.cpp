...
bool GpuExecutor::executeProject(const IRProject& project, EvalContext& ctx, TableResult& out, std::unordered_map<std::string, EvalContext>* tableContexts) {
    ...
    auto ensureActiveRowsCPU = [&]() { ctx.ensureActiveRowsCPU(); };
    ...
    if (shouldCopyFromOut) {
        ...
        for (size_t i = 0; i < out.u32Names.size() && i < out.u32Cols.size(); ++i) {
            savedU32[out.u32Names[i]] = out.u32Cols[i];
        }
        for (size_t i = 0; i < out.f32Names.size() && i < out.f32Cols.size(); ++i) {
            savedF32[out.f32Names[i]] = out.f32Cols[i];
        }
        for (size_t i = 0; i < out.stringNames.size() && i < out.stringCols.size(); ++i) {
            savedString[out.stringNames[i]] = out.stringCols[i];
        }
    ...
    for (size_t i = 0; i < project.exprs.size(); ++i) {
                ...
                if (func.args[0] && func.args[0]->kind == TypedExpr::Kind::Column) {
                    ...
                    if (!ctx.flatStringCols.count(flatKey)) {
                        ...
                        if (sIt == ctx.stringCols.end()) {
                            for (int suffix = 1; suffix <= 9; ++suffix) {
                                std::string sfx = colName + "_" + std::to_string(suffix);
                                ctx.ensureStringCol(sfx);
                                sIt = ctx.stringCols.find(sfx);
        ...
        if (expr->kind == TypedExpr::Kind::Column) {
            ...
            // Materialize from dict if stringCols is empty but dictCol exists
            ctx.ensureStringCol(strLookupCol);
            if (ctx.stringCols.count(strLookupCol)) {
                ...
                if (ctx.rowCount == 0) {
                     ...
                     // Try zero-cost move when possible (no alias needed later)
                ...
                if (!outName.empty()) {
                    ...
                    // Propagate dict encoding if available
            ...
            if (itU != ctx.u32Cols.end()) {
                ...
                if (ctx.rowCount == 0) {
                    ...
                    // GPU gather path: prefer existing GPU buffer over uploading CPU vector
                    auto itGpu = ctx.u32ColsGPU.find(lookupCol);
                    if (itGpu != ctx.u32ColsGPU.end() && itGpu->second) {
                        MTL::Buffer* dst = GpuOps::gatherU32(itGpu->second, ctx.activeRowsGPU, ctx.activeRowsCountGPU);
                ...
                // Only create an alias when there's a real rename (outName != col).
                // Do NOT create un-suffixed aliases for multi-instance resolution
                // (e.g., l_suppkey resolved to l_suppkey_3 but outName is l_suppkey).
                // Those aliases shadow DELIM key columns in join outputs,
                // causing DELIM dedup to use the wrong column data (Q21 bug).
            ...
            if (itF != ctx.f32Cols.end()) {
                ...
                if (ctx.rowCount == 0) {
                    ...
                    if (ctx.activeRowsGPU && ctx.activeRowsCountGPU > 0 && itF->second.size() > ctx.activeRows.size()) {
                        // Prefer existing GPU buffer over uploading CPU vector
                        auto itGpu = ctx.f32ColsGPU.find(lookupCol);
                        if (itGpu != ctx.f32ColsGPU.end() && itGpu->second) {
                            MTL::Buffer* dst = GpuOps::gatherF32(itGpu->second, ctx.activeRowsGPU, ctx.activeRowsCountGPU);
                            colData.resize(ctx.activeRowsCountGPU);
            ...
            if (!outName.empty() && ctx.f32Cols.count(outName)) {
                ...
                ctx.f32Cols[posName] = colData;
                out.f32Cols.push_back(colData); // Copy
            ...
            if (!outName.empty() && ctx.u32Cols.count(outName)) {
                ...
                updateRowCount(colData.size());
                ctx.u32Cols[posName] = colData;
                out.u32Cols.push_back(colData); // Copy
                out.u32Names.push_back(outName);
                continue;
            }
            ...
            MTL::Buffer* gpuBuf = evaluateExpression(expr, ctx);
            ...
            if (!values.empty()) {
                ...
                out.f32Names.push_back(outName.empty() ? posName : outName);
                    ...
                    if (!outName.empty()) {
                        ...
                        if (!found) {
                            // Check f32Cols for suffix match (key ends with outName or vice versa)
                            for (const auto& [key, val] : ctx.f32Cols) {
                                // Require minimum 3 chars to avoid overly broad matches
                            ...
                            // Check u32Cols for suffix match
                            if (!found) {

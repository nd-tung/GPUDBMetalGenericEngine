...
// Uses dictionary IDs when available, otherwise prefix-u64-accelerated CPU ranking.
static MTL::Buffer* buildRankStringStatic(
    ...
    if (dictIt != dictCols.end() && dictIt->second.rowCount == n) {
        ...
        if (asc) {
            if (dict.idsGPU) {
                return s.device()->newBuffer(dict.idsGPU->contents(), n * sizeof(uint32_t), MTL::ResourceStorageModeShared);
            } else {
                return s.device()->newBuffer(dict.ids.data(), n * sizeof(uint32_t), MTL::ResourceStorageModeShared);
            }
        } else {
            if (dict.idsGPU) {
                return GpuOps::invertU32(dict.idsGPU, n);
            } else {
    ...
    // Prefix-u64 accelerated ranking
...
static void gatherNumericColumns(TableResult& table, MTL::Buffer* idxBuf, uint32_t n, bool debug) {
    ...
    std::vector<MTL::Buffer*> gatheredU32, srcU32Bufs;
    ...
    std::vector<MTL::Buffer*> gatheredF32, srcF32Bufs;
    ...
    for (size_t i = 0; i < table.u32Cols.size(); ++i) {
        ...
        table.u32ColsGPU[i].reset(gatheredU32[i]);
    ...
    for (size_t i = 0; i < table.f32Cols.size(); ++i) {
        ...
        table.f32ColsGPU[i].reset(gatheredF32[i]);
...
bool GpuExecutor::executeOrderBy(const IROrderBy& order, TableResult& table,
    ...
    auto buildRankString = [&](const std::vector<std::string>& col, bool asc,
        ...
        return buildRankStringStatic(col, asc, colName, n, dictCols, debug);
    };

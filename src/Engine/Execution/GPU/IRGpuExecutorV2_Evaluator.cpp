...
bool IRGpuExecutorV2::executeGPUFilterRecursive(const TypedExprPtr& expr, EvalContext& ctx) {
    ...
    if (expr->kind == TypedExpr::Kind::Unary) {
        ...
        if (un.op == UnaryOp::Not) {
             ...
             // 3. Perform Set Difference: Input - Result
             // Since we don't have a GPU set diff yet, do it on CPU (consistent with OR implementation)
             std::vector<uint32_t> inputData(inputCount);
             if (inputCount > 0) {
                 if (inputIndices) memcpy(inputData.data(), inputIndices->contents(), inputCount * 4);
                 else {
                     // inputIndices null means 0..inputCount-1 (if inputCount > 0)
                     // But usually if activeRowsGPU is null, we assume standard 0..N ?
                     // The context semantics says: if activeRowsGPU is null, effective rows are 0..rowCount-1.
                     // But here inputCount is what we captured from activeRowsCountGPU.
                     // If activeRowsGPU was null, inputCount is ctx.activeRowsCountGPU which might be 0?
                     // Let's rely on ctx.rowCount if inputIndices is null.
                 }
             }

             // Handle implicit identity (0..N) if inputIndices is null
             if (!inputIndices && MAX(ctx.rowCount, inputCount) > 0) {
                 inputCount = ctx.rowCount; // Fallback
                 inputData.resize(inputCount);
                 for(uint32_t i=0; i<inputCount; ++i) inputData[i] = i;
             } 

             std::vector<uint32_t> resData(resultCount);
             if (resultCount > 0 && resultIndices) memcpy(resData.data(), resultIndices->contents(), resultCount * 4);

             // Prepare output
             std::vector<uint32_t> diffData;
             diffData.reserve(inputCount - resultCount);
             
             // Assume sorted indices (GPU ops generally preserve order or produce sorted indices)
             std::set_difference(inputData.begin(), inputData.end(), 
                                 resData.begin(), resData.end(), 
                                 std::back_inserter(diffData));
             ...
             if (diffData.empty()) {
                 ...
                 ctx.activeRowsGPU = OperatorsGPU::createBuffer(diffData.data(), diffData.size() * sizeof(uint32_t));
                 ctx.activeRowsCountGPU = diffData.size();
             }
             return true;
        }
        return false;
    ...
    if (expr->kind == TypedExpr::Kind::Function) {
        ...
        // Handle LIKE, NOTLIKE, SUFFIX, PREFIX
        if ((fn.name == "LIKE" || fn.name == "NOTLIKE" || fn.name == "SUFFIX" || fn.name == "PREFIX") && fn.args.size() == 2) {
    ...
    if (expr->kind == TypedExpr::Kind::Binary) {
        ...
        if (bin.op == BinaryOp::Or) {
             ...
             MTL::Buffer* leftRes = ctx.activeRowsGPU; // New buffer
             ...
             // ctx takes ownership, but we already retained it for local hold?
             // Actually inputIndices was owned by ctx initially.
             // We retained it -> refcount=2 (if owned).
             // executeGPUFilterRecursive(Left) released it (if modified ctx) -> refcount=1.
             // So now ctx.activeRowsGPU is leftRes.
             // We want to restore inputIndices. 
             // We assign ctx.activeRowsGPU = inputIndices.
             // inputIndices has refcount 1. ctx owns it now. Correct.
             ctx.activeRowsCountGPU = inputCount;
             
             // 4. Run Right
             if (!executeGPUFilterRecursive(bin.right, ctx)) {
                  if (leftRes) leftRes->release();
                  // inputIndices is now owned by ctx (or released if execute failed inside)
                  // Wait, if execute(Right) failed, it might have left ctx dirty.
                  // But usually failure means exception or return false.
                  // If it returns false, it might have already messed up.
                  // But we are returning false anyway.
                  // We just need cleanup leftRes.
                  // inputIndices is managed by ctx / execute.
                  return false;
             }
             ...
             // 5. Cleanup Input Indices (our local hold logic is tricky here)
             // Actually, after executing Right, ctx has rightRes.
             // inputIndices was released by execute(Right) if it modified ctx.
             // So we don't hold inputIndices anymore effectively unless we retained twice?
             // Let's rely on retained 'leftRes' and 'rightRes'.
             
             // 6. Union leftRes and rightRes (Host side)
             std::vector<uint32_t> lData(leftCount);
             if (leftCount > 0 && leftRes) memcpy(lData.data(), leftRes->contents(), leftCount * 4);
             
             std::vector<uint32_t> rData(rightCount);
             if (rightCount > 0 && rightRes) memcpy(rData.data(), rightRes->contents(), rightCount * 4);
             ...
             // STL Set Union
             std::vector<uint32_t> unionData;
             unionData.reserve(leftCount + rightCount);
             std::set_union(lData.begin(), lData.end(), rData.begin(), rData.end(), std::back_inserter(unionData));

             // 7. Upload result
             if (ctx.activeRowsGPU) ctx.activeRowsGPU->release(); // Release Right result
             
             if (unionData.empty()) {
                 ctx.activeRowsGPU = OperatorsGPU::createBuffer(nullptr, 4); // Dummy
                 ctx.activeRowsCountGPU = 0;
             } else {
                 ctx.activeRowsGPU = OperatorsGPU::createBuffer(unionData.data(), unionData.size() * sizeof(uint32_t));
                 ctx.activeRowsCountGPU = unionData.size();
             }
             return true;
        }
...
MTL::Buffer* IRGpuExecutorV2::evalExprFloatGPU(const TypedExprPtr& expr, EvalContext& ctx) {
    ...
    if (expr->kind == TypedExpr::Kind::Column) {
        ...
        } else if (ctx.u32ColsGPU.count(col)) {

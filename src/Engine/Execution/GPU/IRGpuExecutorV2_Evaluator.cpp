...
#include "OperatorsGPU.hpp" // Included for OperatorsGPU::filter*
...
bool IRGpuExecutorV2::executeGPUFilterRecursive(const TypedExprPtr& expr, EvalContext& ctx) {
    ...
    if (debug) std::cerr << "[V2] DEBUG REC: Kind=" << (int)expr->kind << "\n";
    ...
    if (expr->kind == TypedExpr::Kind::Unary) {
        ...
        if (un.op == UnaryOp::Not) {
             ...
             uint32_t totalRows = ctx.rowCount;
             ...
             if (inputIndices) {
                 inputMask = OperatorsGPU::indicesToMask(inputIndices, inputCount, totalRows);
             } else {
                 // All rows active - create all-ones mask on GPU
                 inputMask = OperatorsGPU::createFilledU32(1, totalRows);
             }

             MTL::Buffer* resultMask = OperatorsGPU::indicesToMask(resultIndices, resultCount, totalRows);
             
             // diffMask = inputMask AND NOT resultMask
             MTL::Buffer* diffMask = OperatorsGPU::logicAndNotU32(inputMask, resultMask, totalRows);
             
             // Compact mask back to indices
             auto [diffIndices, diffCount] = OperatorsGPU::compactU32Mask(diffMask, totalRows);
    ...
    if (expr->kind == TypedExpr::Kind::Function) {
        ...
        if (debug) std::cerr << "[V2] DEBUG FUNC: " << fn.name << " args=" << fn.args.size() << "\n";
        
        // Handle broken IN function (Parser/Planner workaround)
        if (fn.name == "IN") {
            ...
            if (fn.args.size() == 1) {
                 ...
                 if (arg0 && arg0->kind == TypedExpr::Kind::Column && arg0->asColumn().column == "...") {
                     if (debug) std::cerr << "[V2] WARNING: Ignoring truncated IN (...) filter. Assuming handled by scan." << std::endl;
                     return true;
                 }
            }
            ...
            if (debug) std::cerr << "[V2] DEBUG IN: left kind=" << (int)left->kind << "\n";
            
            TypedExprPtr root = nullptr;
            for (size_t i = 1; i < fn.args.size(); ++i) {
                const TypedExpr* right = unwrap(fn.args[i].get());
                if (debug) std::cerr << "[V2] DEBUG IN: arg " << i << " kind=" << (int)right->kind << "\n";
        ...
        if ((fn.name == "LIKE" || fn.name == "NOTLIKE" || fn.name == "SUFFIX" || fn.name == "PREFIX" || fn.name == "CONTAINS") && fn.args.size() == 2) {
             ...
             if (left->kind == TypedExpr::Kind::Column && right->kind == TypedExpr::Kind::Literal) {
                 ...
                 if (!vec && debug) {
                     std::cerr << "[V2] DEBUG: String lookup failed for " << colName << ". Available keys: ";
                 ...
                 if (vec) {
                      if (debug) {
                          std::cerr << "[V2] Found string col " << colName << " size " << vec->size() << " pattern '" << pat << "'" << std::endl;
                      }
                      
                      std::optional<FilterResultGPU> res;
                      if (fn.name == "PREFIX") {
                          res = OperatorsGPU::filterStringPrefix(colName, *vec, pat, false);
                      } else {
                          res = OperatorsGPU::filterString(colName, *vec, op, pat);
                      }
                      
                      if (res) {
                          if (debug) std::cerr << "[V2] String Filter Result Count: " << res->count << "\n";
                          // Handle context intersection (GPU Join logic)
                          if (ctx.activeRowsGPU) {
                              if (debug) std::cerr << "[V2] Intersecting with existing " << ctx.activeRowsCountGPU << " rows\n";
                              auto joinRes = OperatorsGPU::joinHash(
                                  ctx.activeRowsGPU, nullptr, ctx.activeRowsCountGPU,
                                  res->indices, nullptr, res->count
                              );
                              if (debug) std::cerr << "[V2] Intersection Result: " << joinRes.count << " rows\n";
                              
                              MTL::Buffer* newActive = OperatorsGPU::gatherU32(ctx.activeRowsGPU, joinRes.buildIndices, joinRes.count);
                              
                              if (ctx.activeRowsGPU) ctx.activeRowsGPU->release(); // Was missing release? No, checked below. But safer.
                              // ctx.activeRowsGPU->release() was done in lines below in original code?
                              // Actually my pasted code in previous turn:
                              // ctx.activeRowsGPU->release();
                              // res->indices->release();
                              // Correct.
                              
                              if (res->indices) res->indices->release(); // wait, res->indices is activeRowsGPU for the other branch
    ...
    if (expr->kind == TypedExpr::Kind::Binary) {
        ...
        if (bin.op == BinaryOp::Or) {
             ...
             MTL::Buffer* leftMask = OperatorsGPU::indicesToMask(leftRes, leftCount, totalRows);
             MTL::Buffer* rightMask = OperatorsGPU::indicesToMask(rightRes, rightCount, totalRows);
             
             MTL::Buffer* unionMask = OperatorsGPU::logicOrU32(leftMask, rightMask, totalRows);
             
             // Compact mask back to indices
             auto [unionIndices, unionCount] = OperatorsGPU::compactU32Mask(unionMask, totalRows);
    ...
    if (expr->kind == TypedExpr::Kind::Compare) {
        ...
        if (leftRaw && rightRaw && 
            ...
            if (debug) std::cerr << "[V2] DEBUG: Literal vs Literal comparison\n";
            bool result = false;
            ...
            if (result) {
                ...
                // Clear all rows
                if(ctx.activeRowsGPU) ctx.activeRowsGPU->release();
                ctx.activeRowsGPU = OperatorsGPU::createBuffer(nullptr, 4);
        ...
        if (leftRaw && rightRaw && 
            ...
            rightRaw->kind == TypedExpr::Kind::Column) {
             ...
             if (!lActual.empty() && !rActual.empty()) {
                  if (debug) std::cerr << "[V2] Col vs Col: " << lActual << " vs " << rActual << "\n";
                  ...
                  if (useF32 && !lIsF32) {
                      finalA = OperatorsGPU::castU32ToF32(rootA, ctx.rowCount);
                      freeFinalA = true;
                  }
                  if (useF32 && !rIsF32) {
                      finalB = OperatorsGPU::castU32ToF32(rootB, ctx.rowCount);
                      freeFinalB = true;
                  }

                  // 2. Gather if active rows
                  if (ctx.activeRowsGPU) {
                       MTL::Buffer* gA = (useF32) ? OperatorsGPU::gatherF32(finalA, ctx.activeRowsGPU, workingCount) 
                                                  : OperatorsGPU::gatherU32(finalA, ctx.activeRowsGPU, workingCount);
                       if (freeFinalA) finalA->release(); // Release intermediate
                       finalA = gA;
                       freeFinalA = true; // Gathered buffer needs release

                       MTL::Buffer* gB = (useF32) ? OperatorsGPU::gatherF32(finalB, ctx.activeRowsGPU, workingCount) 
                                                  : OperatorsGPU::gatherU32(finalB, ctx.activeRowsGPU, workingCount);
                  ...
                  // 3. Execute
                  std::optional<FilterResultGPU> res;
                  if (useF32) res = OperatorsGPU::filterColColF32(finalA, finalB, workingCount, opInt);
                  else res = OperatorsGPU::filterColColU32(finalA, finalB, workingCount, opInt);
                  ...
                  if (res) {
                      if (ctx.activeRowsGPU) {
                           // Combine indices
                           MTL::Buffer* newActive = OperatorsGPU::gatherU32(ctx.activeRowsGPU, res->indices, res->count);
                           if (ctx.activeRowsGPU) ctx.activeRowsGPU->release();
        ...
        if (cmp.op == engine::CompareOp::Like || cmp.op == engine::CompareOp::Eq) {
             ...
             if (left->kind == TypedExpr::Kind::Column && right->kind == TypedExpr::Kind::Literal && 
                 ...
                 if (vec) {
                      ...
                      auto res = OperatorsGPU::filterString(colName, *vec, op, pat);
                      
                      if (res) {
                          if (ctx.activeRowsGPU) {
                              auto joinRes = OperatorsGPU::joinHash( ctx.activeRowsGPU, nullptr, ctx.activeRowsCountGPU, res->indices, nullptr, res->count);
                              MTL::Buffer* newActive = OperatorsGPU::gatherU32(ctx.activeRowsGPU, joinRes.buildIndices, joinRes.count);
        ...
        if (cmp.op == engine::CompareOp::Eq || cmp.op == engine::CompareOp::Ne) {
             ...
             if (left->kind == TypedExpr::Kind::Column && right->kind == TypedExpr::Kind::Literal && 
                 ...
                 if (!actualCol.empty()) {
                     if (std::getenv("GPUDB_DEBUG_OPS")) {
                          std::cerr << "[V2] Found StringHash col " << actualCol << " for pattern '" << pat << "' (hashing)" << std::endl;
                     }
                     uint32_t hashVal = OperatorsGPU::fnv1a32(pat);
                     ...
                     std::optional<FilterResultGPU> res;
                     if (ctx.activeRowsGPU) {
                         res = OperatorsGPU::filterU32Indexed(actualCol, colBuf, ctx.activeRowsGPU, count, op, hashVal);
                     } else {
                         res = OperatorsGPU::filterU32(actualCol, colBuf, count, op, hashVal);
                     }
        ...
        if (cmp.op == engine::CompareOp::In) {
             ...
             if (!isCol) {
                 if (debug) {
                     std::cerr << "[V2] IN only supported on Columns. Kind=" << (int)leftExprRaw->kind << "\n";
                     if (leftExprRaw->kind == TypedExpr::Kind::Function) {
                         std::cerr << "[V2] Function: " << leftExprRaw->asFunction().name << "\n";
                         // Debug available string cols if we failed to find base col
                         std::cerr << "[V2] Available string cols: ";
                         for(const auto& kv : ctx.stringCols) std::cerr << kv.first << " ";
                         std::cerr << "\n";
                     }
                 }
        ...
        if (debug) std::cerr << "[V2] DEBUG CMP Kinds: " << (int)leftUnwrapped->kind << " vs " << (int)rightUnwrapped->kind << "\n";
        ...
        // Handle "lit = col"
            ...
            switch (op) {
             ...
             if (debug) std::cerr << "[V2] DEBUG ColCol: " << c1 << " vs " << c2 << "\n";
             ...
             if (!b1 || !b2) {
                 if (debug) {
                     if (!b1) std::cerr << "[V2] Failed to resolve " << c1 << "\n";
                     if (!b2) std::cerr << "[V2] Failed to resolve " << c2 << "\n";
             ...
             if (ctx.activeRowsGPU) {
                 input1 = f1 ? OperatorsGPU::gatherF32(b1, ctx.activeRowsGPU, currentCount)
                             : OperatorsGPU::gatherU32(b1, ctx.activeRowsGPU, currentCount);
                 temp.push_back(input1);
                 
                 input2 = f2 ? OperatorsGPU::gatherF32(b2, ctx.activeRowsGPU, currentCount)
                             : OperatorsGPU::gatherU32(b2, ctx.activeRowsGPU, currentCount);
                 temp.push_back(input2);
             }
             
             std::optional<FilterResultGPU> res;
             if (f1 || f2) {
                 if (!f1) { MTL::Buffer* t = OperatorsGPU::castU32ToF32(input1, currentCount); temp.push_back(t); input1 = t; }
                 if (!f2) { MTL::Buffer* t = OperatorsGPU::castU32ToF32(input2, currentCount); temp.push_back(t); input2 = t; }
                 res = OperatorsGPU::filterColColF32(input1, input2, currentCount, (int)op);
             } else {
                 res = OperatorsGPU::filterColColU32(input1, input2, currentCount, (int)op);
             }
             ...
             if (debug) std::cerr << "[V2] GPU Filter: Generic Expression Path\n";
             ...
             if (leftIsLit && rightBuf) {
                 ...
                 if(valid) res = OperatorsGPU::filterF32("expr", rightBuf, count, flipped, leftLitVal);
             } 
             else if (leftBuf && rightIsLit) {
                 res = OperatorsGPU::filterF32("expr", leftBuf, count, op, rightLitVal);
             } 
             else if (leftBuf && rightBuf) {
                 res = OperatorsGPU::filterColColF32(leftBuf, rightBuf, count, (int)op);
             }
             ...
             if (res) {
                 if (debug) std::cerr << "[V2] Generic Filter Result (likely p_size): " << res->count << " rows\n";
                 if (ctx.activeRowsGPU) {
                      if (debug) std::cerr << "[V2] Intersecting Generic with " << ctx.activeRowsCountGPU << " rows\n";
                      MTL::Buffer* newActive = OperatorsGPU::gatherU32(ctx.activeRowsGPU, res->indices, res->count);
                      ctx.activeRowsGPU->release();
                      ctx.activeRowsGPU = newActive;
                      res->indices->release();
        ...
        if (debug) std::cerr << "[V2] GPU Filter checking col: " << colName << "\n";
        ...
        if (strVec) {
             ...
             auto res = OperatorsGPU::filterString(colName, *strVec, op, pat);
             if (res) {
                  if (ctx.activeRowsGPU) {
                      auto joinRes = OperatorsGPU::joinHash(
                          ctx.activeRowsGPU, nullptr, ctx.activeRowsCountGPU,
        ...
        if (ctx.f32ColsGPU.count(colName)) {
            ...
            if (debug) std::cerr << "[V2] DEBUG FIND BUF: colName " << colName << " not found in GPU cols\n";
            ...
            if (!buf) {
                if (debug) std::cerr << "[V2] DEBUG FIND BUF: FAILED for colName " << colName << "\n";
                return false;
            }
        }
        ...
        if (isF32) {
            ...
            try {
                if (ctx.activeRowsGPU) {
                    res = OperatorsGPU::filterF32Indexed(colName, buf, ctx.activeRowsGPU, currentCount, op, val);
                } else {
                    res = OperatorsGPU::filterF32(colName, buf, currentCount, op, val);
                }
            } catch(...) {
                if(debug) std::cerr << "[V2] Exception in filterF32\n";
                res = std::nullopt;
            }
            
            if (!res && debug) {
                std::cerr << "[V2] DEBUG: filterF32 failed. bufLen=" << buf->length() << " count=" << currentCount << " val=" << val << " op=" << (int)op << "\n";
            }
            
            if (!res) {
                 if(debug) std::cerr << "[V2] GPU Filter F32 failed for " << colName << ". CPU fallback disabled.\n";
            ...
            if (buf && buf->length() == sizeof(uint32_t) && currentCount > 1) {
                if (debug) std::cerr << "[V2] DEBUG: Scalar broadcast detected. bufLen=" << buf->length() << " currentCount=" << currentCount << "\n";
            ...
            if (debug) std::cerr << "[V2] DEBUG: About to filter. activeRowsGPU=" << (ctx.activeRowsGPU ? "set" : "null") << " bufLen=" << buf->length() << " count=" << currentCount << " val=" << val << " op=" << (int)op << "\n";

            if (ctx.activeRowsGPU) {
                res = OperatorsGPU::filterU32Indexed(colName, buf, ctx.activeRowsGPU, currentCount, op, val);
            } else {
                res = OperatorsGPU::filterU32(colName, buf, currentCount, op, val);
            }
            if (!res && debug) {
                std::cerr << "[V2] DEBUG: filterU32 failed. bufLen=" << buf->length() << " count=" << currentCount << " val=" << val << " op=" << (int)op << "\n";
            }
            
            if (!res) {
                 if(debug) std::cerr << "[V2] GPU Filter U32 failed for " << colName << ". CPU fallback disabled.\n";
        ...
        if (res) {
            ...
            if (debug && res->indices) {
                ...
                std::cerr << "[V2] Filter result indices first 5: ";
    ...
    if (expr->kind == TypedExpr::Kind::Column) {
        ...
        if (debug) std::cerr << "[V2] GPU Filter checking col (boolean): " << colName << "\n";
        ...
        if (!buf && debug) {
             std::cerr << "[V2] DEBUG: Bool Col Lookup Failed: '" << colName << "'\n";
        ...
        if (buf) {
             ...
             std::optional<FilterResultGPU> res;
             if (isF32) {
                 if (ctx.activeRowsGPU) res = OperatorsGPU::filterF32Indexed(colName, buf, ctx.activeRowsGPU, currentCount, engine::expr::CompOp::NE, 0.0f);
                 else res = OperatorsGPU::filterF32(colName, buf, currentCount, engine::expr::CompOp::NE, 0.0f);
             } else {
                 if (ctx.activeRowsGPU) res = OperatorsGPU::filterU32Indexed(colName, buf, ctx.activeRowsGPU, currentCount, engine::expr::CompOp::NE, 0);
                 else res = OperatorsGPU::filterU32(colName, buf, currentCount, engine::expr::CompOp::NE, 0);
             }
        ...
        if (colName.rfind("NOT prefix(", 0) == 0) {
             ...
             if (comma != std::string::npos && endParen != std::string::npos && comma > 11) {
                 ...
                 if (debug) std::cerr << "[V2] Corrected Q16: " << c << " NOT LIKE " << pat << "%\n";
                 ...
                 if (vec) {
                      ...
                      std::optional<FilterResultGPU> res = OperatorsGPU::filterStringPrefix(c, *vec, pat, true);
    ...
    if (expr->kind == TypedExpr::Kind::Function) {
        ...
        if ((func.name == "contains" || func.name == "CONTAINS") && func.args.size() >= 2) {
            ...
            if (c->kind == TypedExpr::Kind::Column && l->kind == TypedExpr::Kind::Literal) {
                 ...
                 if (vec) {
                      auto res = OperatorsGPU::filterString(colName, *vec, engine::expr::CompOp::EQ, pat);
                      if (res) {
                          if (ctx.activeRowsGPU) ctx.activeRowsGPU->release();
                          ctx.activeRowsGPU = res->indices;
                          ctx.activeRowsCountGPU = res->count;
...
// Stubs
std::vector<float> IRGpuExecutorV2::evalExprFloat(const TypedExprPtr&, const EvalContext&) {
    throw std::runtime_error("CPU evalExprFloat called. CPU fallback is strictly disabled.");
    return {};
}

std::vector<uint32_t> IRGpuExecutorV2::evalExprU32(const TypedExprPtr&, const EvalContext&) {
    throw std::runtime_error("CPU evalExprU32 called. CPU fallback is strictly disabled.");
    return {};
}

std::vector<bool> IRGpuExecutorV2::evalPredicate(const TypedExprPtr&, const EvalContext&) {
    throw std::runtime_error("CPU evalPredicate called. CPU fallback is strictly disabled.");
    return {};
}
MTL::Buffer* IRGpuExecutorV2::evalExprFloatGPU(const TypedExprPtr& expr, EvalContext& ctx) {
    ...
    if (expr->kind == TypedExpr::Kind::Column) {
        ...
        if (isAggCol && ctx.rowCount > 1) {
            ...
            for (const auto& [name, b] : ctx.f32ColsGPU) {
                ...
                if (colLower == nameLower) {
                    ...
                    if (varying) {
                        if (debug) std::cerr << "[V2] evalExprFloatGPU: agg match varying col '" << name << "' for '" << col << "'\n";
                        buf = b;
                        break;
                    }
                }
        ...
        if (!buf && ctx.f32ColsGPU.count(col)) {
            ...
            if (vec.size() == 1 && count > 1) { // Scalar broadcast
                float val = vec[0];
                MTL::Buffer* outBuf = OperatorsGPU::createBuffer(nullptr, count * sizeof(float));
                float* ptr = (float*)outBuf->contents();
                std::fill(ptr, ptr + count, val);
                return outBuf;
            } else {
                MTL::Buffer* rawBuf = OperatorsGPU::createBuffer(vec.data(), vec.size() * sizeof(float));
                if (ctx.activeRowsGPU) {
                    MTL::Buffer* gathered = OperatorsGPU::gatherF32(rawBuf, ctx.activeRowsGPU, count);
                    rawBuf->release();
                    return gathered;
                }
                return rawBuf;
            ...
            if (vec.size() == 1 && count > 1) { // Scalar broadcast
                ...
                MTL::Buffer* rawBuf = OperatorsGPU::createBuffer(vec.data(), vec.size() * sizeof(uint32_t));
                MTL::Buffer* targetBuf = rawBuf;
                bool temp = false;
                
                if (ctx.activeRowsGPU) {
                    targetBuf = OperatorsGPU::gatherU32(rawBuf, ctx.activeRowsGPU, count);
                    temp = true;
                }
                
                MTL::Buffer* f32Buf = OperatorsGPU::castU32ToF32(targetBuf, count);
        ...
        if (!buf) {
            ...
            if (env_truthy("GPUDB_DEBUG_OPS") && col.find("sum") != std::string::npos) {
                std::cerr << "[V2] evalExprFloatGPU: fuzzy search for col='" << col << "' length=" << col.length() << " normalized length=" << colLower.length() << "\n";
            }
            ...
            for (const auto& [name, b] : ctx.f32ColsGPU) {
                ...
                if (colLower == nameLower) {
                    if (env_truthy("GPUDB_DEBUG_OPS")) std::cerr << "[V2] evalExprFloatGPU: fuzzy matched '" << col << "' to '" << name << "'\n";
                    buf = b;
                    break;
                }
            }
            ...
            if (!buf) {
                for (const auto& [name, b] : ctx.u32ColsGPU) {
                    ...
                    nameLower.erase(std::remove_if(nameLower.begin(), nameLower.end(), ::isspace), nameLower.end());
                    if (colLower == nameLower) {
                        if (env_truthy("GPUDB_DEBUG_OPS")) std::cerr << "[V2] evalExprFloatGPU: fuzzy matched (u32) '" << col << "' to '" << name << "'\n";
                        buf = b;
                        isU32 = true;
                        break;
                    }
        ...
        if (!buf) {
            ...
            if (hasHash0) {
                if (col.find("sum(") != std::string::npos || col.find("SUM(") != std::string::npos ||
                    ...
                    std::string posKey = "#" + std::to_string(g_aggregateCounter);
                    if (env_truthy("GPUDB_DEBUG_OPS")) std::cerr << "[V2] evalExprFloatGPU: heuristic mapping " << col << " to " << posKey << "\n";
        ...
        if (isU32) {
             if (ctx.activeRowsGPU) {
                 ...
                 MTL::Buffer* gathered = OperatorsGPU::gatherU32(buf, ctx.activeRowsGPU, count);
                 MTL::Buffer* casted = OperatorsGPU::castU32ToF32(gathered, count);
                 gathered->release(); // Release gathering intermediate
                 return casted;
             } else {
                 return OperatorsGPU::castU32ToF32(buf, count);
             }
        }
        
        // If F32
        if (ctx.activeRowsGPU) {
            return OperatorsGPU::gatherF32(buf, ctx.activeRowsGPU, count);
        } else {
            buf->retain();
            return buf; 
        }
    ...
    if (expr->kind == TypedExpr::Kind::Aggregate) {
        ...
        if (hasScalarAggregates) {
             ...
             if (ctx.u32ColsGPU.count(posKey)) {
                 g_aggregateCounter++;
                 return OperatorsGPU::castU32ToF32(ctx.u32ColsGPU[posKey], count);
             }
        }
    ...
    if (expr->kind == TypedExpr::Kind::Compare) {
        ...
        if (env_truthy("GPUDB_DEBUG_OPS")) {
              ...
              std::cerr << "[V2] Eval Compare: LeftKind=" << (c.left ? (int)c.left->kind : -1) << " RightKind=" << (c.right ? (int)c.right->kind : -1) << "\n";
        ...
        if (executeGPUFilterRecursive(std::const_pointer_cast<TypedExpr>(expr), subCtx)) {
            ...
            if (subCtx.activeRowsCountGPU > 0) {
                 ...
                 OperatorsGPU::scatterConstantF32(outBuf, subCtx.activeRowsGPU, subCtx.activeRowsCountGPU, 1.0f);
            }
            ...
            if (env_truthy("GPUDB_DEBUG_OPS")) std::cerr << "[V2] Eval Compare: executeGPUFilterRecursive failed\n";
            if (subCtx.activeRowsGPU) subCtx.activeRowsGPU->release();
            return nullptr;
        }
    }
    ...
    if (expr->kind == TypedExpr::Kind::Binary) {
        ...
        if (bin.op == BinaryOp::Mul || bin.op == BinaryOp::Sub || bin.op == BinaryOp::Add || bin.op == BinaryOp::Div) {
            ...
            if (leftIsLit) {
                ...
                if (isMul) return OperatorsGPU::arithMulF32ColScalar(rightBuf, leftVal, count);
                else if (isAdd) return OperatorsGPU::arithAddF32ColScalar(rightBuf, leftVal, count); // Commutative
                else if (isDiv) return OperatorsGPU::arithDivF32ScalarCol(leftVal, rightBuf, count);
                else return OperatorsGPU::arithSubF32ScalarCol(leftVal, rightBuf, count);
            } else if (rightIsLit) {
                // Left op Lit
                MTL::Buffer* leftBuf = evalExprFloatGPU(bin.left, ctx);
                if (!leftBuf) return nullptr;
                if (isMul) return OperatorsGPU::arithMulF32ColScalar(leftBuf, rightVal, count);
                else if (isAdd) return OperatorsGPU::arithAddF32ColScalar(leftBuf, rightVal, count);
                else if (isDiv) return OperatorsGPU::arithDivF32ColScalar(leftBuf, rightVal, count);
                else return OperatorsGPU::arithSubF32ColScalar(leftBuf, rightVal, count);
            } else {
                // Left op Right
                MTL::Buffer* leftBuf = evalExprFloatGPU(bin.left, ctx);
                MTL::Buffer* rightBuf = evalExprFloatGPU(bin.right, ctx);
                if (!leftBuf || !rightBuf) return nullptr;
                if (isMul) return OperatorsGPU::arithMulF32ColCol(leftBuf, rightBuf, count);
                else if (isAdd) return OperatorsGPU::arithAddF32ColCol(leftBuf, rightBuf, count);
                else if (isDiv) return OperatorsGPU::arithDivF32ColCol(leftBuf, rightBuf, count);
                else return OperatorsGPU::arithSubF32ColCol(leftBuf, rightBuf, count);
            }
        }
    }
    ...
    if (expr->kind == TypedExpr::Kind::Case) {
        if (debug) std::cerr << "[V2] evalExprFloatGPU: Entering CASE expression handler\n";
        
        // Safely access CaseExpr data
        if (!std::holds_alternative<CaseExpr>(expr->data)) {
            std::cerr << "[V2] ERROR: CASE expression kind but data is not CaseExpr!\n";
            return nullptr;
        }
        const auto& c = expr->asCase();
        
        if (debug) {
            std::cerr << "[V2] CASE: " << c.cases.size() << " WHEN branches, hasElse=" << (c.elseExpr ? "yes" : "no") << "\n";
        }
        ...
        if (c.elseExpr) {
            if (debug) std::cerr << "[V2] CASE: elseExpr kind=" << static_cast<int>(c.elseExpr->kind) << "\n";
            if (c.elseExpr->kind == TypedExpr::Kind::Literal) {
                ...
                outBuf = OperatorsGPU::createBuffer(nullptr, count * sizeof(float));
        ...
        for (const auto& w : c.cases) {
             ...
             if (containsErrorFunction(w.then)) {
                 if (debug) std::cerr << "[V2] CASE: skipping WHEN branch with error() guard function\n";
                 continue;
             }
             ...
             if (executeGPUFilterRecursive(w.when, subCtx)) {
                 ...
                 if (subCtx.activeRowsCountGPU > 0) {
                     if (literalThen) {
                         OperatorsGPU::scatterConstantF32(outBuf, subCtx.activeRowsGPU, subCtx.activeRowsCountGPU, thenVal);
                     } else {
                         MTL::Buffer* thenBuf = evalExprFloatGPU(w.then, subCtx);
                         if (thenBuf) {
                             OperatorsGPU::scatterF32(thenBuf, outBuf, subCtx.activeRowsGPU, subCtx.activeRowsCountGPU);
                             thenBuf->release();
                         } else {
                             // If evaluation fails (e.g. error function), and we have active rows, we can't proceed on GPU
                             if (debug) std::cerr << "[V2] CASE THEN non-literal GPU eval failed\n";
                             if (subCtx.activeRowsGPU) subCtx.activeRowsGPU->release();
                             outBuf->release();
                             return nullptr;
                         }
                 ...
                 if (debug) std::cerr << "[V2] CASE condition eval failed on GPU\n";
    ...
    if (expr->kind == TypedExpr::Kind::Function) {
        ...
        for (const auto& candidate : candidates) {
            if (ctx.f32ColsGPU.count(candidate)) {
                ...
                if (ctx.activeRowsGPU) {
                     ...
                     return OperatorsGPU::createBuffer(buf->contents(), count * sizeof(float)); // Copy
                }
            }
            if (ctx.u32ColsGPU.count(candidate)) {
                ...
                if (ctx.activeRowsGPU) {
                     MTL::Buffer* gathered = OperatorsGPU::gatherU32(buf, ctx.activeRowsGPU, count);
                     MTL::Buffer* asFloat = OperatorsGPU::castU32ToF32(gathered, count);
                     gathered->release();
                     return asFloat;
                } else {
                     return OperatorsGPU::castU32ToF32(buf, count);
        ...
        if (fnName == "FIRST" || fnName == "\"FIRST\"") {
            if (debug) std::cerr << "[V2] evalExprFloatGPU: FIRST aggregate, evaluating argument\n";
            if (fn.args.size() >= 1 && fn.args[0]) {
                ...
                for (const auto& [colName, buf] : ctx.f32ColsGPU) {
                    ...
                    if (colLower.find("\"first\"") == 0 || colLower.find("first(") == 0) {
                        ...
                        if (uniform) {
                            if (debug) std::cerr << "[V2] evalExprFloatGPU: FIRST using scalar column '" << colName << "' val=" << ptr[0] << "\n";
                            buf->retain();
                            return buf;
                        }
                    }
        ...
        if (fnName == "MIN" || fnName == "MAX" || fnName == "SUM" || fnName == "AVG" || fnName == "COUNT") {
            ...
            for (const auto& [colName, buf] : ctx.f32ColsGPU) {
                ...
                if (lowerCol.find(fnPrefix) == 0 && buf) {
                    if (debug) std::cerr << "[V2] evalExprFloatGPU: Found aggregate col '" << colName << "' for " << fnName << "\n";
                    buf->retain();
                    return buf;
                }
            }
            ...
            for (const auto& [colName, buf] : ctx.u32ColsGPU) {
                ...
                std::transform(lowerCol.begin(), lowerCol.end(), lowerCol.begin(), ::tolower);
                if (lowerCol.find(fnPrefix) == 0 && buf) {
                    if (debug) std::cerr << "[V2] evalExprFloatGPU: Found aggregate col (u32) '" << colName << "' for " << fnName << ", casting to f32\n";
                    if (ctx.activeRowsGPU) {
                        MTL::Buffer* gathered = OperatorsGPU::gatherU32(buf, ctx.activeRowsGPU, count);
                        MTL::Buffer* casted = OperatorsGPU::castU32ToF32(gathered, count);
                        gathered->release();
                        ...
                        return OperatorsGPU::castU32ToF32(buf, buf->length() / sizeof(uint32_t));
                    }
                }
            }
            ...
            std::string posKey = "#" + std::to_string(g_aggregateCounter);
             if (debug) std::cerr << "[V2] evalExprFloatGPU: Function heuristic mapping " << fnName << " to " << posKey << "\n";
        ...
        if (fnName == "ERROR" || fnName == "\"ERROR\"") {
             MTL::Buffer* outBuf = OperatorsGPU::createBuffer(nullptr, count * sizeof(float));
             float* ptr = (float*)outBuf->contents();
             std::fill(ptr, ptr + count, 0.0f); // Dummy 0.0 to prevent crash, caller (CASE) should handle validity
             return outBuf;
        }
        ...
        if (fnName == "EXTRACT" && fn.args.size() == 2) {
             ...
             if (unitStr == "YEAR") {
                 ...
                 if (!inBuf) {
                     if (debug) {
                         std::cerr << "[V2] EXTRACT failed: could not evaluate valArg. Kind=" << (int)valArg->kind << "\n";
                 ...
                 MTL::Buffer* divBuf = OperatorsGPU::arithDivF32ColScalar(inBuf, 10000.0f, count);
                 MTL::Buffer* floorBuf = OperatorsGPU::mathFloorF32(divBuf, count);
        ...
        if (debug) std::cerr << "[V2] Unsupported GPU function: " << fn.name << "\n";
        return nullptr; 
    }
...
MTL::Buffer* IRGpuExecutorV2::evalExprU32GPU(const TypedExprPtr& expr, EvalContext& ctx) {
    ...
    if (expr->kind == TypedExpr::Kind::Column) {
        std::string col = expr->asColumn().column;
        // col = resolveColumnAlias(col); // Handle Q15 alias (FIXME: Implement properly on GPU or share linkage)
        ...
        if (ctx.u32ColsGPU.count(col)) {
            ...
            if (vec.size() >= count) { // Full column
                MTL::Buffer* outBuf = OperatorsGPU::createBuffer(nullptr, count * sizeof(uint32_t));
                memcpy(outBuf->contents(), vec.data(), count * sizeof(uint32_t));
                return outBuf; // Direct return, handles activeRowsGPU logic below if I restructure? 
                // Ah, buf logic below handles gather. If I return here, I miss gather.
                // Better to set 'buf' if I can keep it alive. 
                // But buf is MTL::Buffer*. If I create one here, who releases it?
                // The caller assumes ownership if I return it. 
                // If I set 'buf', the block below `if (buf)` handles gather. 
                // But `buf` in ctx.u32ColsGPU is usually owned by ctx. 
                // If I create a new one, I must ensure it's released or I must return effectively.
                
                // Let's Just return gathered/copied buffer directly.
                if (ctx.activeRowsGPU) {
                     MTL::Buffer* tmp = OperatorsGPU::createBuffer(nullptr, count * sizeof(uint32_t)); 
                     memcpy(tmp->contents(), vec.data(), count * sizeof(uint32_t));
                     // Gather
                     MTL::Buffer* gathered = OperatorsGPU::gatherU32(tmp, ctx.activeRowsGPU, count);
                     tmp->release();
                     return gathered;
                } else {
                     MTL::Buffer* outBuf = OperatorsGPU::createBuffer(nullptr, count * sizeof(uint32_t)); 
        ...
        if (buf) {
            if (ctx.activeRowsGPU) {
                return OperatorsGPU::gatherU32(buf, ctx.activeRowsGPU, count);
            } else {
    ...
    if (expr->kind == TypedExpr::Kind::Literal) {
        ...
        const auto& lit = expr->asLiteral();
        if (std::holds_alternative<int64_t>(lit.value)) val = (uint32_t)std::get<int64_t>(lit.value);
        else if (std::holds_alternative<std::string>(lit.value)) val = OperatorsGPU::fnv1a32(std::get<std::string>(lit.value));
        else if (std::holds_alternative<double>(lit.value)) val = (uint32_t)std::get<double>(lit.value);
    ...
    if (debug) std::cerr << "[V2] evalExprU32GPU not implemented for this kind: " << (int)expr->kind << "\n";
    return nullptr;
}
} // namespace engine

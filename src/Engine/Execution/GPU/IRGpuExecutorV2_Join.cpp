...
bool IRGpuExecutorV2::executeJoin(const IRJoinV2& join, const std::string& datasetPath,
    ...
    if (lCount > 0 && rCount > 0) {
        if (isCrossJoin) {
            ...
            uint32_t* pPtr = (uint32_t*)jRes.probeIndices->contents();
            uint32_t* bPtr = (uint32_t*)jRes.buildIndices->contents();
            
            std::vector<uint32_t> lIndices(lCount);
            if (leftCtx.activeRowsGPU) {
                std::memcpy(lIndices.data(), leftCtx.activeRowsGPU->contents(), lCount * sizeof(uint32_t));
            } else {
                std::iota(lIndices.begin(), lIndices.end(), 0);
            }
            
            std::vector<uint32_t> rIndices(rCount);
            if (rightCtx.activeRowsGPU) {
                std::memcpy(rIndices.data(), rightCtx.activeRowsGPU->contents(), rCount * sizeof(uint32_t));
            } else {
                std::iota(rIndices.begin(), rIndices.end(), 0);
            }
            
            size_t idx = 0;
            // Cross product
            for (uint32_t i = 0; i < lCount; ++i) {
                uint32_t lVal = lIndices[i];
            ...
            if (debug) std::cerr << "[V2] Multi-Key Join (2 keys)\n";
        ...
        if (!isCrossJoin && debug) std::cerr << "[V2] GPU Join: Build Right (" << rCount << "), Probe Left (" << lCount << ")\n";
        
        if (!isCrossJoin) {
            if (resolvedKeys.size() == 2) {
                 jRes = OperatorsGPU::joinHashU64(rBuf, rightCtx.activeRowsGPU, rCount, lBuf, leftCtx.activeRowsGPU, lCount);
                 lBuf->release(); rBuf->release();
            } else {
                 jRes = OperatorsGPU::joinHash(rBuf, rightCtx.activeRowsGPU, rCount, lBuf, leftCtx.activeRowsGPU, lCount);
            }
        }
    } else {
    ...
    if (resCount == 0) {
        for (const auto& [name, _] : leftCtx.u32Cols) outCtx.u32Cols[name] = {};
        for (const auto& [name, _] : leftCtx.f32Cols) outCtx.f32Cols[name] = {};
        for (const auto& [name, _] : leftCtx.stringCols) outCtx.stringCols[name] = {};
        for (const auto& [name, _] : rightCtx.u32Cols) outCtx.u32Cols[name] = {};
        for (const auto& [name, _] : rightCtx.f32Cols) outCtx.f32Cols[name] = {};
        for (const auto& [name, _] : rightCtx.stringCols) outCtx.stringCols[name] = {};
        return true;
    }
    ...
    for (const auto& [name, valid] : leftCtx.u32Cols) {
        ...
        if (src) {
             MTL::Buffer* gathered = OperatorsGPU::gatherU32(src, jRes.probeIndices, resCount);
             outCtx.u32ColsGPU[name] = gathered;
             outCtx.u32Cols[name].clear(); // Mark as GPU-only. TODO: keep CPU if needed?
        }
    }
    for (const auto& [name, valid] : leftCtx.f32Cols) {
        ...
        if (src) {
             MTL::Buffer* gathered = OperatorsGPU::gatherF32(src, jRes.probeIndices, resCount);
    ...
    if (rCount > 0) {
        for (const auto& [name, valid] : rightCtx.u32Cols) {
            ...
            if (src) {
                 MTL::Buffer* gathered = OperatorsGPU::gatherU32(src, jRes.buildIndices, resCount);
                 outCtx.u32ColsGPU[name] = gathered;
        ...
        for (const auto& [name, valid] : rightCtx.f32Cols) {
            ...
            if (src) {
                 MTL::Buffer* gathered = OperatorsGPU::gatherF32(src, jRes.buildIndices, resCount);
                 outCtx.f32ColsGPU[name] = gathered;
                 outCtx.f32Cols[name].clear();
            }
        }
    ...
    if (!leftCtx.stringCols.empty() || !rightCtx.stringCols.empty()) {
        ...
        for (const auto& [name, vec] : leftCtx.stringCols) {
            ...
            newVec.reserve(resCount);
            // Verify vec size? vec should be original table size. Indices are within range.
            for(uint32_t idx : cpuProbeIndices) {
                if(idx < vec.size()) newVec.push_back(vec[idx]);
                else newVec.push_back(""); // Error fallback
            }
        ...
        for (const auto& [name, vec] : rightCtx.stringCols) {
             std::vector<std::string> newVec;
             newVec.reserve(resCount);
             for(uint32_t idx : cpuBuildIndices) {
                if(idx < vec.size()) newVec.push_back(vec[idx]);
                else newVec.push_back("");
             }
             outCtx.stringCols[name] = std::move(newVec);
        }
    }

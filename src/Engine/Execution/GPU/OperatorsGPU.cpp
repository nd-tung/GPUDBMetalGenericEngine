...
std::optional<FilterResultGPU> OperatorsGPU::filterString(const std::string& colName,
    ...
    if (std::getenv("GPUDB_DEBUG_OPS")) std::cerr << "[V2] GPU filterString: rowCount=" << rowCount << " pattern=" << pattern << "\n";
...
JoinResultGPU OperatorsGPU::joinHashU64(MTL::Buffer* buildKeys, 
    ...
    // U64 Keys
    auto bufHTKeys = store.device()->newBuffer(capacity * sizeof(uint64_t), MTL::ResourceStorageModeShared);
    auto bufHTVals = store.device()->newBuffer(capacity * sizeof(uint32_t), MTL::ResourceStorageModeShared);
    auto bufHTLocks = store.device()->newBuffer(capacity * sizeof(uint32_t), MTL::ResourceStorageModeShared);
    
    // Init Keys to EMPTY (0xFFFFFFFFFFFFFFFF)
    std::memset(bufHTKeys->contents(), 0xFF, capacity * sizeof(uint64_t));
    // Init Locks to 0
    std::memset(bufHTLocks->contents(), 0, capacity * sizeof(uint32_t));
    
    auto p_build = makePSO(store.device(), store.library(), "ops::join_build_u64");
    if (!p_build) {
        bufHTKeys->release(); bufHTVals->release(); bufHTLocks->release();
        return {nullptr, nullptr, 0};
    }
    ...
    if (!p_probe) {
        bufHTKeys->release(); bufHTVals->release(); outBuildIndices->release(); outProbeIndices->release(); outCount->release();
        return {nullptr, nullptr, 0};
    }
...
MTL::Buffer* OperatorsGPU::gatherU32(MTL::Buffer* in, MTL::Buffer* indices, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::gatherF32(MTL::Buffer* in, MTL::Buffer* indices, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();

...
uint32_t OperatorsGPU::fnv1a32(std::string_view s) {
    ...
    while (std::getline(file, line)) {
        ...
        while (e != std::string::npos) {
            if (col == columnIndex) {
                ...
                token.erase(0, token.find_first_not_of(" \t\n\r"));
                token.erase(token.find_last_not_of(" \t\n\r") + 1);
                data.push_back(OperatorsGPU::fnv1a32(token));
                break;
            }
            s = e + 1;
            e = line.find('|', s);
...
uint32_t OperatorsGPU::encodeStringForColumn(const std::string& table, const std::string& col, const std::string& val) {
...
RelationGPU OperatorsGPU::scanTable(const std::string& dataset_path,
...
std::optional<FilterResultGPU> OperatorsGPU::filterU32(const std::string& colName,
                                                      MTL::Buffer* col,
                                                      uint32_t rowCount,
    ...
    if (std::getenv("GPUDB_DEBUG_OPS")) {
        std::cerr << "[V2] GPU filterU32: col=" << colName << " rowCount=" << rowCount << " val=" << literal << "\n";
    }
...
std::optional<FilterResultGPU> OperatorsGPU::filterString(const std::string& colName,
    ...
    if (rowCount == 0) return FilterResultGPU{nullptr, 0};
    if (std::getenv("GPUDB_DEBUG_OPS")) {
        std::cerr << "[V2] GPU filterString: rowCount=" << rowCount << " pattern=" << pattern << "\n";
    }
    ...
    if (patternLen > 0) {
        bufPattern = store.device()->newBuffer(rawPattern.data(), patternLen, MTL::ResourceStorageModeShared);
    } else {
    ...
    // 4. Dispatch Kernel
    if (std::getenv("GPUDB_DEBUG_OPS")) std::cerr << "[V2] GPU filterString: dispatching kernel " << totalChars << " chars\n";
    
    const char* kernelName = "ops::filter_string_contains";
    switch(op) {
        ...
        default: break; // Default to contains for LIKE? Or assume caller handles?
                        // If op is LIKE or CONTAINS, keep default.
    }
    ...
    auto filterEnd = std::chrono::high_resolution_clock::now();
    KernelTimer::instance().record("ops::filter_string_contains", "filter", 
        std::chrono::duration<double, std::milli>(filterEnd - filterStart).count(), rowCount);
...
std::optional<FilterResultGPU> OperatorsGPU::filterStringPrefix(const std::string& colName,
                                                          const std::vector<std::string>& data,
                                                          const std::string& pattern,
                                                          bool invert) {
    auto& store = ColumnStoreGPU::instance();
    if (!store.device() || !store.library() || !store.queue()) {
        std::cerr << "[OperatorsGPU] Device/Lib/Queue invalid\n";
        return std::nullopt;
    }
    ...
    // Debug print
    if (std::getenv("GPUDB_DEBUG_OPS")) {
        std::cerr << "[OperatorsGPU] filterStringPrefix pattern='" << pattern << "' invert=" << invert << " rowCount=" << rowCount << "\n";
    }
    ...
    if (std::getenv("GPUDB_DEBUG_OPS")) {
        std::cerr << "[OperatorsGPU] Requesting kernel: " << kernelName << "\n";
    }
    ...
    if (!p_filter || !p_compact) {
        if(!p_filter) std::cerr << "[OperatorsGPU] Failed to make PSO for " << kernelName << "\n";
        if(!p_compact) std::cerr << "[OperatorsGPU] Failed to make PSO for compact\n";
        bufChars->release();
        bufOffsets->release();
        bufLengths->release();
        bufPattern->release();
...
JoinResultGPU OperatorsGPU::joinHash(MTL::Buffer* buildKeys, 
    ...
    // Allocate HT in Shared memory for initialization
    // Ideally use Private and a kernel init, but Shared+memset is easier
    ...
    // Init Keys to EMPTY (0xFFFFFFFF)
    std::memset(bufHTKeys->contents(), 0xFF, capacity * sizeof(uint32_t));
    
    // 2. Build Phase
    auto p_build = makePSO(store.device(), store.library(), "ops::join_build_u32");
    if (!p_build) {
        bufHTKeys->release(); bufHTVals->release();
    ...
    
    {
        ...
        enc->setBuffer(buildIndices, 0, 1); // Can be null
        enc->setBuffer(bufHTKeys, 0, 2);
    ...
    // 3. Probe Phase
    // Allocate Output Buffers (Max = Probe Count)
    ...
    auto p_probe = makePSO(store.device(), store.library(), "ops::join_probe_u32");
    if (!p_probe) {
        bufHTKeys->release(); bufHTVals->release();
...

    {
        ...
        KernelTimer::instance().record("ops::join_probe_u32", "hash_join_probe_u32", 
...
JoinResultGPU OperatorsGPU::joinHashU64(MTL::Buffer* buildKeys, 
                                        MTL::Buffer* buildIndices, 
                                        uint32_t buildCount,
                                        MTL::Buffer* probeKeys,
                                        MTL::Buffer* probeIndices,
...
MTL::Buffer* OperatorsGPU::packU32ToU64(MTL::Buffer* c1, MTL::Buffer* c2, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
std::optional<FilterResultGPU> OperatorsGPU::filterU32Indexed(const std::string& colName,
                                                              MTL::Buffer* col,
                                                              MTL::Buffer* indices,
                                                              uint32_t count,
                                                              engine::expr::CompOp op,
    ...
    if (debug) std::cerr << "[V2] GPU filterU32Indexed: col=" << colName << " rowCount=" << count << " val=" << literal << " result=" << res.count << "\n";
...
std::optional<FilterResultGPU> OperatorsGPU::filterF32(const std::string& colName,
                                                      MTL::Buffer* col,
                                                      uint32_t rowCount,
                                                      engine::expr::CompOp op,
                                                      float literal) {
...
std::optional<FilterResultGPU> OperatorsGPU::filterF32Indexed(const std::string& colName,
                                                              MTL::Buffer* col,
                                                              MTL::Buffer* indices,
                                                              uint32_t count,
                                                              engine::expr::CompOp op,
...
MTL::Buffer* OperatorsGPU::gatherU32(MTL::Buffer* in, MTL::Buffer* indices, uint32_t count, bool sync) {
    auto& store = ColumnStoreGPU::instance();
    auto p_g = makePSO(store.device(), store.library(), "ops::gather_col_u32");
    if (!p_g) {
        std::cerr << "OperatorsGPU::gatherU32: Failed to create PSO ops::gather_col_u32\n";
        return nullptr;
    }

    if (!in || !indices) {
        std::cerr << "OperatorsGPU::gatherU32: Input or Indices buffer is NULL\n";
        return nullptr;
    }

    auto out = store.device()->newBuffer(count * sizeof(uint32_t), MTL::ResourceStorageModeShared);
    if (!out) {
        std::cerr << "OperatorsGPU::gatherU32: Failed to allocate output buffer size " << (count * sizeof(uint32_t)) << "\n";
        return nullptr;
    }
    auto start = std::chrono::high_resolution_clock::now();
    {
...
MTL::Buffer* OperatorsGPU::gatherF32(MTL::Buffer* in, MTL::Buffer* indices, uint32_t count, bool sync) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::castU32ToF32(MTL::Buffer* in, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
RelationGPU OperatorsGPU::applySelection(RelationGPU&& rel, const FilterResultGPU& sel) {
    RelationGPU out;
...
std::optional<JoinMapGPU> OperatorsGPU::hashJoinU32(MTL::Buffer* leftKey,
                                                   uint32_t leftCount,
                                                   MTL::Buffer* rightKey,
                                                   uint32_t rightCount) {
    auto& store = ColumnStoreGPU::instance();
...
RelationGPU OperatorsGPU::materializeJoin(RelationGPU&& left,
                                         RelationGPU&& right,
...
MTL::Buffer* OperatorsGPU::computeRevenue(MTL::Buffer* extendedprice,
                                         MTL::Buffer* discount,
                                         uint32_t rowCount) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::computeCharge(MTL::Buffer* extendedprice,
                                         MTL::Buffer* discount,
                                         MTL::Buffer* tax,
                                         uint32_t rowCount) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::copyAddU32(MTL::Buffer* in, uint32_t count, uint32_t add) {
    auto& store = ColumnStoreGPU::instance();
...
std::optional<GroupByHashTableGPU> OperatorsGPU::groupBySumMultiKey(const std::vector<MTL::Buffer*>& keyColsU32,
                                                                   MTL::Buffer* aggF32,
                                                                   uint32_t rowCount) {
    auto& store = ColumnStoreGPU::instance();
    if (!store.device() || !store.library() || !store.queue()) return std::nullopt;
...
std::optional<GroupByHashTableGPU> OperatorsGPU::groupBySumCountMultiKey(const std::vector<MTL::Buffer*>& keyColsU32,
                                                                         MTL::Buffer* aggF32,
                                                                         uint32_t rowCount) {
    auto& store = ColumnStoreGPU::instance();
    if (!store.device() || !store.library() || !store.queue()) return std::nullopt;
...
std::optional<GroupByHashTableGPU> OperatorsGPU::groupByCountMultiKey(const std::vector<MTL::Buffer*>& keyColsU32,
                                                                      uint32_t rowCount) {
    auto& store = ColumnStoreGPU::instance();
    if (!store.device() || !store.library() || !store.queue()) return std::nullopt;
...
std::optional<GroupByHashTableGPU> OperatorsGPU::groupByAggMultiKeyTyped(const std::vector<MTL::Buffer*>& keyColsU32,
...
void OperatorsGPU::release(FilterResultGPU& r) {
    if (r.indices) r.indices->release();
    r.indices = nullptr;
    r.count = 0;
}

void OperatorsGPU::release(JoinMapGPU& j) {
...
void OperatorsGPU::release(GroupByHashTableGPU& g) {
...
std::vector<std::string> OperatorsGPU::loadStringColumnRaw(const std::string& dataset_path,
                                                           const std::string& table,
...
std::optional<FilterResultGPU> OperatorsGPU::filterColColU32(
    MTL::Buffer* colA,
    MTL::Buffer* colB,
    uint32_t count,
    int opInt) {
...
std::optional<FilterResultGPU> OperatorsGPU::filterColColF32(
    MTL::Buffer* colA,
    MTL::Buffer* colB,
    uint32_t count,
    int opInt) {
...
MTL::Buffer* OperatorsGPU::cmpColColU32Mask(MTL::Buffer* colA, MTL::Buffer* colB, uint32_t count, int op) {
...
MTL::Buffer* OperatorsGPU::cmpColLitU32Mask(MTL::Buffer* colA, uint32_t valB, uint32_t count, int op) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
MTL::Buffer* OperatorsGPU::logicOrU32(MTL::Buffer* colA, MTL::Buffer* colB, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
MTL::Buffer* OperatorsGPU::logicAndU32(MTL::Buffer* colA, MTL::Buffer* colB, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
MTL::Buffer* OperatorsGPU::logicNotU32(MTL::Buffer* mask, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
MTL::Buffer* OperatorsGPU::logicAndNotU32(MTL::Buffer* colA, MTL::Buffer* colB, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
MTL::Buffer* OperatorsGPU::indicesToMask(MTL::Buffer* indices, uint32_t indexCount, uint32_t totalRows) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
...
void OperatorsGPU::clearMask(MTL::Buffer* mask, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
std::pair<MTL::Buffer*, uint32_t> OperatorsGPU::compactU32Mask(MTL::Buffer* mask, uint32_t totalRows) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
void OperatorsGPU::fillU32(MTL::Buffer* buf, uint32_t val, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
void OperatorsGPU::fillF32(MTL::Buffer* buf, float val, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
MTL::Buffer* OperatorsGPU::createFilledU32(uint32_t val, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
...
MTL::Buffer* OperatorsGPU::createFilledF32(float val, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
...
MTL::Buffer* OperatorsGPU::iotaU32(uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
void OperatorsGPU::crossProduct(MTL::Buffer* left, MTL::Buffer* right,
...
MTL::Buffer* OperatorsGPU::selectU32(MTL::Buffer* mask, MTL::Buffer* t, MTL::Buffer* f, uint32_t count) {
    auto* dev = ColumnStoreGPU::instance().device();
    auto* lib = ColumnStoreGPU::instance().library();
    auto* cmd = ColumnStoreGPU::instance().queue()->commandBuffer();
    auto* enc = cmd->computeCommandEncoder();
...
std::optional<FilterResultGPU> OperatorsGPU::filterF32Between(const std::string& colName,
                                                              MTL::Buffer* col,
                                                              uint32_t rowCount,
                                                              float minVal,
                                                              float maxVal) {
...
std::optional<FilterResultGPU> OperatorsGPU::filterF32BetweenIndexed(const std::string& colName,
                                                                     MTL::Buffer* col,
                                                                     MTL::Buffer* indices,
                                                                     uint32_t count,
                                                                     float minVal,
...
std::optional<FilterResultGPU> OperatorsGPU::filterU32Between(const std::string& colName,
                                                              MTL::Buffer* col,
                                                              uint32_t rowCount,
                                                              uint32_t minVal,
                                                              uint32_t maxVal) {
...
std::optional<FilterResultGPU> OperatorsGPU::filterU32BetweenIndexed(const std::string& colName,
                                                                     MTL::Buffer* col,
                                                                     MTL::Buffer* indices,
                                                                     uint32_t count,
                                                                     uint32_t minVal,
...
std::optional<FilterResultGPU> OperatorsGPU::hashJoinSemiU32(MTL::Buffer* leftKey,
                                                             uint32_t leftCount,
                                                             MTL::Buffer* rightKey,
                                                             uint32_t rightCount) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithMulF32ColCol(MTL::Buffer* colA, MTL::Buffer* colB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithMulF32ColColIndexed(MTL::Buffer* colA, MTL::Buffer* colB, MTL::Buffer* indices, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithMulF32ColScalar(MTL::Buffer* colA, float valB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithMulF32ColScalarIndexed(MTL::Buffer* colA, float valB, MTL::Buffer* indices, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithDivF32ColCol(MTL::Buffer* colA, MTL::Buffer* colB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithDivF32ColScalar(MTL::Buffer* colA, float valB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithDivF32ScalarCol(float valA, MTL::Buffer* colB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithSubF32ColCol(MTL::Buffer* colA, MTL::Buffer* colB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithSubF32ColScalar(MTL::Buffer* colA, float valB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithSubF32ScalarCol(float valA, MTL::Buffer* colB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::createBuffer(const void* data, size_t size) {
    auto& store = ColumnStoreGPU::instance();
...
float OperatorsGPU::reduceSumF32(MTL::Buffer* in, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
float OperatorsGPU::reduceMinF32(MTL::Buffer* in, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
float OperatorsGPU::reduceMaxF32(MTL::Buffer* in, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::extractYearFromDate(MTL::Buffer* dateCol, uint32_t count) {
...
MTL::Buffer* OperatorsGPU::arithAddF32ColCol(MTL::Buffer* colA, MTL::Buffer* colB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithAddF32ColScalar(MTL::Buffer* colA, float valB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
MTL::Buffer* OperatorsGPU::arithAddF32ScalarCol(float valA, MTL::Buffer* colB, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();
...
void OperatorsGPU::scatterConstantF32(MTL::Buffer* output, MTL::Buffer* indices, uint32_t indexCount, float val) {
    if (indexCount == 0 || !output || !indices) return;

    auto& store = ColumnStoreGPU::instance();
    // Assuming the kernel is named ops::scatter_constant_f32, so we look up "scatter_constant_f32" 
    // and makePSO prepends "ops::" usually? 
    // Let's check makePSO implementation or usage. 
    // Existing usage: makePSO(dev, lib, "arith_add_f32_scalar_col") -> ops::arith...
...
void OperatorsGPU::scatterF32(MTL::Buffer* input, MTL::Buffer* output, MTL::Buffer* indices, uint32_t count) {
...
MTL::Buffer* OperatorsGPU::mathFloorF32(MTL::Buffer* col, uint32_t count) {
    auto& store = ColumnStoreGPU::instance();

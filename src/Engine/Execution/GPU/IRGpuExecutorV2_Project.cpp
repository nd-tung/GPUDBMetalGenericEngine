...
bool IRGpuExecutorV2::executeProject(const IRProjectV2& project, EvalContext& ctx, TableResult& out, std::unordered_map<std::string, EvalContext>* tableContexts) {
    ...
    // Sync activeRows from GPU if needed (crucial for string column projection)
    if (ctx.activeRowsGPU && ctx.activeRows.size() != ctx.activeRowsCountGPU) {
        if (debug) std::cerr << "[V2] Project: Syncing activeRowsGPU (" << ctx.activeRowsCountGPU << ") to CPU activeRows\n";
    ...
    for (size_t i = 0; i < project.exprs.size(); ++i) {
        ...
        if (expr->kind == TypedExpr::Kind::Function) {
            ...
            if (isYearFunc) {
                ...
                if (colArg && colArg->kind == TypedExpr::Kind::Column) {
                    ...
                    // Helper to find actual column key in map
                    auto findKey = [&](const std::string& target) -> std::string {
                        if (ctx.u32Cols.count(target)) return target;
                        if (ctx.columnAliases.count(target)) {
                            std::string alias = ctx.columnAliases.at(target);
                            if (ctx.u32Cols.count(alias)) return alias;
                        }
                    ...
                    std::string actualKey = findKey(colName);
        ...
        if (expr->kind == TypedExpr::Kind::Column) {
            ...
            // Try to download from GPU if missing on CPU
    ...
    if (rowCountInitialized) {
        ...
        size_t fallbackCount = ctx.activeRows.empty() ? originalRowCount : ctx.activeRows.size();

...
bool IRGpuExecutorV2::executeGroupBy(const IRGroupByV2& groupBy, EvalContext& ctx, TableResult& out) {
    ...
    // Expected row count for this GroupBy
    ...
    // Verify key sizes match expected row count
    for (const auto& kv : keyVecs) {
        if (kv.size() != expectedRowCount) {
            if (debug) {
                std::cerr << "[V2] GroupBy: key size mismatch, expected " << expectedRowCount 
                          << " but got " << kv.size() << "\n";
            }
    ...
    if (useGpu && !keyVecs.empty()) {
        ...
        if (store.device() && store.library() && store.queue()) {
            ...
            // Determine row count from key vectors

    ...
    for (size_t nodeIdx = 0; nodeIdx < plan.nodes.size(); ++nodeIdx) {
        ...
        switch (node.type) {
            ...
            case IRNodeV2::Type::GroupBy: {
                ...
                // Update context with the grouped output for subsequent operators
                // IMPORTANT: Clear old columns - GroupBy produces entirely new data
                ...
                currentCtx.currentTable.clear();  // No longer tied to a single table
                
                // After a GroupBy, clear joinedTables to allow re-joining with tables
                // This is needed for DuckDB's decorrelation pattern where a SEMI join
                // after a GroupBy needs to correlate back to the outer table
                // BUT: Keep a marker that we have grouped data that should be saved
                // if a new pipeline starts (for scalar subquery patterns)
                // We insert a special marker to indicate there's grouped data
                joinedTables.clear();
                joinedTables.insert("__GROUPED__");  // Marker for grouped pipeline
                ...
                for(const auto& [name, vec] : currentCtx.u32Cols) {
                    if (!vec.empty()) {
                         ...
                         if (buf) {
                            ...
                            // createBuffer returns with ref count 1, so we don't need to retain if we pass ownership
                            // But Scan does buf->retain(). Let's follow OperatorsGPU::createBuffer semantics.
                            // If OperatorsGPU::createBuffer matches newBuffer, it returns retained.
                            // The map will hold the pointer.
                         } else {
            ...
            case IRNodeV2::Type::Aggregate: {
                ...
                // But createBuffer gave me +1. 
                // I assigned to posKey. If I retain for posKey, I have +2.
                // Is map ownership transferring or weak?
                // Scan does: ctx.u32ColsGPU[name] = buf; buf->retain();
                // This implies map doesn't own, or map holds raw ptr and we rely on retain to keep it alive until ctx clears?
                // Actually if Scan retains, and createBuffer returns +1.
                // Scan: rel.u32Cols[name] (buf) -> createBuffer (+1).
                // assignment. buf->retain() (+2).
                // rel destructor releases (+1).
                // ctx survives.
                // Here, I createBuffer (+1).
                // I assign to f32ColsGPU[posKey].
                // If I don't retain, ref is 1.
                // If I retain, ref is 2.
                // Who releases?
                // If result of createBuffer is dropped (local var goes out of scope), no one releases.
                // So I pass ownership to ctx?
                // ctx usually doesn't have a destructor that releases all GPU buffers automatically unless I see it doing so.
                // In Scan: buf->retain().
                // I will follow Scan pattern: retain for each assignment. 
                // But I also need to release the local 'aggBuf' ref?
                // Actually createBuffer returns an autoreleased buffer in ObjC?
                // In C++ Metal-cpp, `newBuffer` returns retained. `OperatorsGPU::createBuffer` wraps it.
                // If `OperatorsGPU::createBuffer` returns retained, I have 1 ref.
                // I assign to map. Map holds pointer.
                // If I retain again (+2), and I lose local var, I have a leak if ctx doesn't release 2 refs.
                // I suspect ctx destructor manages release of map values?
                // Or ctx is POD?
                // Let's assume manual management pattern: I transfer ownership to map.
                // Scan codes: buf->retain() suggests ctx *DOES* release, or Scan *DOES* release.
                // I'll stick to: create, assign. But since I assign to multiple keys, I must retain for subsequent keys.
                // For the first key, I can transfer (no retain if I release local). 
                // Scan does buf->retain().
                // I'll assume createBuffer returns +1.
                // I invoke aggBuf->release() at end of block?
                // Safe bet: retain for every map insertion. Then release aggBuf (local) once.

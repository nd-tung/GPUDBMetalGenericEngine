...
std::map<size_t, ScanInstance> buildScanInstanceMap(const PlanV2& plan) {
    ...
    for (size_t i = 0; i < plan.nodes.size(); ++i) {
        if (plan.nodes[i].type == IRNodeV2::Type::Scan) {
        ...
        switch (node.type) {
            ...
            case IRNodeV2::Type::Join: {
                ...
                if (std::getenv("GPUDB_DEBUG_OPS")) {
                     std::cerr << "[V2] DEBUG: Join collected cols:";
...
static void collectPatternMatchColumns(const TypedExprPtr& expr, 
    ...
    if (expr->kind == TypedExpr::Kind::Function) {
        ...
        if (isStringComp) {
            ...
            auto registerCol = [&](const TypedExprPtr& e) {
                if (e && e->kind == TypedExpr::Kind::Column) {
                    ...
                    std::string table = tableForColumn(colName);
                    if (std::getenv("GPUDB_DEBUG_OPS")) {
                        std::cerr << "[V2] DEBUG: Found string comparison col " << colName << " table=" << table << "\n";
                    }
                    if (!table.empty()) {
                        tableCols[table].insert(colName);
                    }
        ...
        // Check for mangled function call in column name (legacy/Planner quirk support)
        if (colName.rfind("NOT prefix(", 0) == 0) {
            ...
            if (comma != std::string::npos && comma > 11) {
                ...
                if (!table.empty()) {
                    ...
                    if (std::getenv("GPUDB_DEBUG_OPS")) {
                        std::cerr << "[V2] DEBUG: Found mangled prefix comparison col " << c << " table=" << table << "\n";
                    }
                }
            }
        }
...
std::unordered_map<std::string, std::set<std::string>> collectPatternMatchColumnsV2(const PlanV2& plan) {
    ...
    for (const auto& node : plan.nodes) {
        switch (node.type) {
            ...
            case IRNodeV2::Type::Project:
                for (size_t i = 0; i < node.asProject().exprs.size(); ++i) {
                    ...
                    if (!outName.empty() && expr->kind == TypedExpr::Kind::Column) {
                        ...
                        if (std::getenv("GPUDB_DEBUG_OPS")) {
                            std::cerr << "[V2] DEBUG: Tracked pattern alias " << outName << " -> " << src << "\n";
                        }
                    }
                }
                break;
            ...
            case IRNodeV2::Type::GroupBy: {
                const auto& gb = node.asGroupBy();
                for (const auto& k : gb.keys) collectPatternMatchColumns(k, tableCols, aliasMap);
...
void IRGpuLoader::loadTables(
    ...
    for (const auto& [tableName, cols] : tableColsMap) {
        if (debug) {
            std::cerr << "[V2] DEBUG: Loading table " << tableName << " with cols:";
        ...
        if (multiInstanceTables.count(tableName)) {
            for (const auto& [nodeIdx, inst] : scanInstanceMap) {
                if (inst.baseTable == tableName) {
                    RelationGPU rel = OperatorsGPU::scanTable(datasetPath, tableName, colVec);
                    if (!rel.rowCount) {
                    ...
                    if (pmIt != patternMatchCols.end()) {
                        for (const auto& colName : pmIt->second) {
                            ...
                            auto rawStrings = OperatorsGPU::loadStringColumnRaw(datasetPath, tableName, actualCol);
                            if (!rawStrings.empty()) {
                                if (inst.instanceNum == 1) {
                    ...
                    if (debug) {
                        std::cerr << "[V2] Loaded instance " << inst.instanceKey 
                                  << " (" << rel.rowCount << " rows)\n";
                    }
                }
            }
            ...
            if (pmIt != patternMatchCols.end()) {
                for (const auto& colName : pmIt->second) {
                    ...
                    if (!rawStrings.empty()) {
                        ...
                        if (debug) {
                            std::cerr << "[V2] Loaded raw strings for " << tableName << "." << colName 

    ...
    uint gid [[thread_position_in_grid]]) 
{
    ...
    // If no explicit indices provided, use gid
    ...
    uint gid [[thread_position_in_grid]]) 
{
    ...
    for (uint32_t i = 0; i < MAX_HASH_STEPS; ++i) {
        // Spinlock
        while (atomic_exchange_explicit(&ht_locks[idx], 1u, memory_order_relaxed) == 1u) {
            // spin
        }
        
        // Critical section
        uint64_t existing = ht_keys[idx];
        if (existing == EMPTY_KEY64) {
            ht_keys[idx] = key;
            ht_vals[idx] = payload;
            atomic_store_explicit(&ht_locks[idx], 0u, memory_order_relaxed);
            return;
...
kernel void join_probe_u64(
    ...
    const device uint64_t* ht_keys [[buffer(2)]],
    const device uint32_t* ht_vals [[buffer(3)]],
    constant uint32_t& ht_capacity [[buffer(4)]],
    constant uint32_t& row_count [[buffer(5)]],
    device atomic_uint* out_counter [[buffer(6)]],
    ...
    uint gid [[thread_position_in_grid]]) 
{
    ...
    for (uint32_t i = 0; i < MAX_HASH_STEPS; ++i) {
        uint64_t k_at = ht_keys[idx];
        if (k_at == EMPTY_KEY64) break;
        if (k_at == key) {
            uint32_t build_idx = ht_vals[idx];
            uint32_t write_pos = atomic_fetch_add_explicit(out_counter, 1, memory_order_relaxed);
            out_build_indices[write_pos] = build_idx;
